...
export PATH="$PATH:$(dirname "${BASH_SOURCE[0]}")/node_modules/.bin"
...
LOCAL_IP='49.233.155.21'
# 去掉域名，直接使用IP访问
DOMAIN_NAME="www.trendbot.cn"
...
cleanup() {
    ...
    if [[ -f "/etc/nginx/sites-enabled/ameeting.conf" ]]; then
        print_status "Removing nginx configuration..."
        sudo rm -f "/etc/nginx/sites-enabled/ameeting.conf" 2>/dev/null || true
...
generate_ssl_certs() {
    ...
    local cert_file="$CERTS_DIR/ameeting.crt"
    local key_file="$CERTS_DIR/ameeting.key"
...
create_nginx_config() {
    ...
    local config_file="/etc/nginx/sites-available/ameeting.conf"
...
server {
    listen 80;
    server_name $LOCAL_IP www.trendbot.cn trendbot.cn;
...
server {
    ...
    # SSL Configuration
    ssl_certificate $CERTS_DIR/ameeting.crt;
    ssl_certificate_key $CERTS_DIR/ameeting.key;
...
EOF
    ...
    # Enable the site
    sudo ln -sf "$config_file" "/etc/nginx/sites-enabled/ameeting.conf"
...
display_access_info() {
    ...
    echo -e "  • Nginx config: ${CYAN}/etc/nginx/sites-available/ameeting.conf${NC}"
    echo ""
...
check_livekit() {
    ...
    livekit-server --dev  --bind 0.0.0.0 --node-ip 49.233.155.21  --keys "devkey: secret" >"$LOG_DIR/livekit.log" 2>&1 &
